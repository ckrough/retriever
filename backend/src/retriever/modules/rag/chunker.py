...
from retriever.modules.rag.schemas import Chunk, DocumentChunker
...
class HierarchicalChunker:
    ...
    Implements the DocumentChunker protocol. Splits documents respecting
    their structure: headers, paragraphs, sentences, with overlap.
    """
...
# Verify HierarchicalChunker satisfies the DocumentChunker protocol
_chunker_check: DocumentChunker = HierarchicalChunker()
...
def _split_paragraph_by_sentences(
    ...
    """
    # Simple sentence splitting (period, question mark, exclamation mark
    # followed by space)
...
def _get_overlap_text(text: str, overlap_size: int) -> str:
    """Get the last overlap_size characters from text at sentence boundaries.
